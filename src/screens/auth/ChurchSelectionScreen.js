...
export default function ChurchSelectionScreen({ navigation }) {
    ...
    const handleProceed = async () => {
        try {
            ...
            const churchWithCorrectKey = { ...selectedChurch, church_id: selectedChurch.id };
            delete churchWithCorrectKey.id;
            await AsyncStorage.setItem('selectedChurch', JSON.stringify(churchWithCorrectKey));

            navigation.replace('DistrictSelection');
        } catch (error) {
            setError(error.message || 'An error occurred while selecting the church.');
        }
    };
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title} maxFontSizeMultiplier={1.2}>Select Your Church</Text>
            ...
            <TouchableOpacity
                style={[
                    styles.selectButton,
                    selectedChurch ? styles.selectButtonActive : styles.selectButtonDisabled,
                ]}
                disabled={!selectedChurch}
                onPress={handleProceed}
            >
                <Text style={styles.selectButtonText} maxFontSizeMultiplier={1.2}>Select Church</Text>
            </TouchableOpacity>
        </View>
    );
}
