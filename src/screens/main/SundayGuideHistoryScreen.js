...
export default function SundayGuideHistoryScreen() {
        ...
        const fetchHistory = async (churchId) => {
            ...
            try {
                ...
                if (historyData.length > 0) {
                    await AsyncStorage.setItem('SundayGuideHistory', JSON.stringify(historyData));
    ...
    const renderHistoryCards = () => {
        ...
        if (days.length === 0) {
            return <Text style={styles.emptyText}>{error || 'No content available'}</Text>;
        }
        ...
        return days.map((day) => (
            <TouchableOpacity
                ...
                onPress={() => navigation.navigate('SundayGuideScreen', { selectedDay: day })}
            >
                <Text style={styles.historyDay}>{day}</Text>
                {/* <Text style={styles.historySubtitle}>{groupedHistory[day].length} item(s)</Text> */}
            </TouchableOpacity>
        ));
    };
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            {loading ? (
                <ActivityIndicator size="large" color={isDarkTheme ? '#fff' : '#000'} style={{ marginTop: 20 }} />
            ) : (
                <ScrollView
                    ref={scrollViewRef}
