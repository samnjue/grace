        ...
        useCallback(() => {
            const refreshNews = async () => {
                ...
                try {
                    ...
                    if (data.length > 0) {
                        setNews(data);
                        await AsyncStorage.setItem(`districtNews_${selectedDistrict}`, JSON.stringify(data));
                    }
                } catch (err) {
    ...
    useEffect(() => {
        const getNews = async () => {
            ...
            try {
                ...
                const cachedNews = await AsyncStorage.getItem(`districtNews_${selectedDistrict}`);
                if (cachedNews) {
