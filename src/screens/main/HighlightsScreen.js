...
const HighlightScreen = ({ navigation }) => {
    ...
    const groupHighlights = (highlights) => {
        const sortedKeys = Object.keys(highlights).sort((a, b) =>
            highlights[b].timestamp - highlights[a].timestamp
        );
        ...
        sortedKeys.forEach((key, index) => {
            ...
            const [prevBook, prevChapter, prevVerse] = prevKey ? prevKey.split('_') : [];

            if (
                currentGroup.length === 0 ||
                (book === prevBook && chapter === prevChapter && parseInt(verse) === parseInt(prevVerse) + 1)
            ) {
    ...
    return (
        <View style={[styles.container, { backgroundColor: isDarkTheme ? '#121212' : '#fff', paddingTop: insets.top }]}>
            ...
            <View style={styles.header}>
                ...
                <Text style={[styles.headerText, { color: isDarkTheme ? '#fff' : '#000' }]}>Highlights</Text>
            </View>

            {/* Highlighted Verses List */}
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                {groupedHighlights.length === 0 ? (
                    <Text style={[styles.emptyText, { color: isDarkTheme ? '#bbb' : '#333' }]}>
                    ...
                    groupedHighlights.map((group, index) => {
                        ...
                        const verseText = group.map((key) => {
                            const [book, chapter, verse] = key.split('_');
                            return (
                                <Text key={key}>
                                    <Text style={styles.verseNumber}>{verse} </Text>
                                    {bibleData?.[book]?.[chapter]?.verses?.[verse] || 'Verse not found'}{' '}
                                </Text>
                            );
                        });

                        // Extract color correctly from the new data structure
                        const highlightData = highlightedVerses[firstVerseKey];
                        const highlightColor = highlightData ? highlightData.color : '#FFD700'; // Default color if missing

                        return (
                            <View key={index} style={[styles.card, { backgroundColor: highlightColor }]}>
                                <Text style={styles.cardTitle}>{book} {chapter}:{verseRange}</Text>
                                <Text style={styles.cardText}>{verseText}</Text>
                            </View>
                        );
                    })
                )}
